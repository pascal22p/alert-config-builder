...
class AlertConfigBuilderSpec
  ...
  "AlertConfigBuilder" should {
    ...
    "Returns None when app config file exists but zone key is absent" in {
      AlertConfigBuilder("service-with-absent-zone-key",
                         handlers = Seq("h1", "h2")).build shouldBe None
    }

    "Returns None when app config file exists but it unparsable" in {
      AlertConfigBuilder("service-with-unparseable-app-config",
                         handlers = Seq("h1", "h2")).build shouldBe None
    }

    "Maps the correct service domain" in {
      val service2Config = AlertConfigBuilder(
        "service2",
        handlers = Seq("h1", "h2")).build.get.parseJson.asJsObject.fields
      val service3Config = AlertConfigBuilder(
        "service3",
        handlers = Seq("h1", "h2")).build.get.parseJson.asJsObject.fields
    ...
    "build/configure http status threshold with given thresholds" in {
      ...
      val serviceConfig =
        AlertConfigBuilder("service1", handlers = Seq("h1", "h2"))
          ...
          .withHttpStatusThreshold(HttpStatusThreshold(HTTP_STATUS_504, 4))
          .build
          .get
          .parseJson
          .asJsObject
    ...
    "build/configure logMessageThresholds with given thresholds" in {
      ...
      val serviceConfig =
        AlertConfigBuilder("service1", handlers = Seq("h1", "h2"))
          ...
          .withLogMessageThreshold("SIMUATED_ERROR3", 5)
          .build
          .get
          .parseJson
          .asJsObject
      ...
      serviceConfig("log-message-thresholds") shouldBe JsArray(
        JsObject("message" -> JsString("SIMUATED_ERROR1"),
                 "count" -> JsNumber(3)),
        JsObject("message" -> JsString("SIMUATED_ERROR2"),
                 "count" -> JsNumber(4)),
        JsObject("message" -> JsString("SIMUATED_ERROR3"),
                 "count" -> JsNumber(5))
      )
    }
    ...
    "build/configure any empty http status threshold" in {
      ...
      val serviceConfig =
        AlertConfigBuilder("service1").build.get.parseJson.asJsObject.fields
