...
case class AlertConfigBuilder(
  ...
  def withExceptionThreshold(exceptionThreshold: Int) =
    this.copy(exceptionThreshold = exceptionThreshold)

  def withHttp5xxThreshold(http5xxThreshold: Int) =
    this.copy(http5xxThreshold = http5xxThreshold)

  def withHttp5xxPercentThreshold(http5xxPercentThreshold: Int) =
    this.copy(http5xxPercentThreshold = http5xxPercentThreshold)
  ...
  def withLogMessageThreshold(message: String, threshold: Int) =
    this.copy(
      logMessageThresholds = logMessageThresholds :+ LogMessageThreshold(
        message,
        threshold))
  ...
  def build: Option[String] = {
    ...
    if (!appConfigDirectory.exists) {
      throw new FileNotFoundException(
        s"Could not find app-config repository: $appConfigPath")
    }

    appConfigFile match {
      case file if !file.exists =>
        logger.info(
          s"No app-config file found for service: '${serviceName}'. File was expected at: '${file.getAbsolutePath}'")
        None
      case file if getZone(file).isEmpty =>
        logger.warn(
          s"app-config file for service: '${serviceName}' does not contain 'zone' key.")
        None
      case file =>
        val serviceDomain = getZone(file)

        ZoneToServiceDomainMapper
          .getServiceDomain(serviceDomain)
          .map(serviceDomain => s"""
             |{
  ...
  def getZone(appConfigFile: File): Option[String] = {
    ...
    parseAppConfigFile match {
      ...
      case Success(appConfigYamlMap) => {
        val appConfig = appConfigYamlMap
...
case class TeamAlertConfigBuilder(
  ...
  def withHttp5xxPercentThreshold(percentThreshold: Double) =
    this.copy(http5xxPercentThreshold = percentThreshold)
...
object ZoneToServiceDomainMapper {
  val logger = new Logger()
  val zoneToServiceMappingFilePath = System.getProperty(
    "zone-mapping-path",
    "zone-to-service-domain-mapping.yml")
  val zoneToServiceMappingFile = new File(zoneToServiceMappingFilePath)
  if (!zoneToServiceMappingFile.exists()) {
    throw new FileNotFoundException(
      s"Could not find zone to service domain mapping file: ${zoneToServiceMappingFilePath}")
  }
